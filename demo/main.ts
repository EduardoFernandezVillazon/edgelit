...
// --- worker mode: forcefield-sim runtime in demo/simWorker.ts ---------------
...
function startWorker(n: number, positions: Float64Array, src: number[], tgt: number[]) {
  client?.terminate()
  client = new SimulationClient(new Worker(new URL('./simWorker.ts', import.meta.url), { type: 'module' }))
  client.onFrame((pos, { tick, alpha }) => {
    ...
    workerStats.simMs = workerStats.lastFrameAt ? workerStats.simMs * 0.9 + (now - workerStats.lastFrameAt) * 0.1 : 0
    workerStats.lastFrameAt = now
    workerStats.tick = tick; workerStats.alpha = alpha; workerStats.framesReceived++
  })
  client.on('error', (m) => console.error('worker:', m))
  const nodes = Array.from({ length: n }, (_, i) => ({ id: `n${i}`, x: positions[2 * i], y: positions[2 * i + 1] }))
  const links = src.map((s, i) => ({ source: `n${s}`, target: `n${tgt[i]}` }))
  workerStats = { tick: 0, alpha: 1, simMs: 0, framesReceived: 0, lastFrameAt: 0, fps: 0 }
  void client.init({ nodes, links, payload: {} })
}
...
function build(n: number): World {
  ...
  if (workerMode) startWorker(n, pos, src, tgt)
...
r.events.on('dragstart', () => { if (!world) return; if (workerMode) client?.alphaTarget(0.3); else { world.sim.setAlphaTarget(0.3); world.running = true } })
r.events.on('drag', ({ node, x, y }) => { if (workerMode) client?.setFixed(node, x, y); else world?.sim.setFixed(node, x, y) })
r.events.on('dragend', ({ node, x, y }) => { if (!world) return; if (workerMode) { client?.setFixed(node, x, y); client?.alphaTarget(0) } else { world.sim.setFixed(node, x, y); world.sim.setAlphaTarget(0) } })
r.events.on('dblclick', ({ node }) => { if (node < 0) return; if (workerMode) client?.setFixed(node, NaN, NaN); else world?.sim.setFixed(node, NaN, NaN) })
...
document.getElementById('toggle')!.onclick = () => { if (!world) return; world.running = !world.running; if (workerMode) client?.run(world.running) }
const modeSel = document.getElementById('mode') as HTMLSelectElement
modeSel.onchange = () => { workerMode = modeSel.value === 'worker'; if (!workerMode) { client?.terminate(); client = null } world = build(+sel.value) }
...
window.demo = { r, build: (n) => { world = build(n) }, world: () => world, setWorkerMode: (on) => { workerMode = on; if (!on) { client?.terminate(); client = null } }, workerStats: () => workerStats }
